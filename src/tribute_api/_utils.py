...
def raise_from_error_tuple(err_tup: tuple[int, TributeError]) -> NoReturn:
    ...
    match status:
        case 400:
            raise TributeApiV1BadRequest(error.error, error.message)
        case 401:
            raise TributeApiV1Unauthorized(error.error, error.message)
        case 403:
            raise TributeApiV1Forbidden(error.error, error.message)
        case 404:
            raise TributeApiV1NotFound(error.error, error.message)
        case 409:
            raise TributeApiV1Conflict(error.error, error.message)
        case 429:
            raise TributeApiV1TooManyRequests(error.error, error.message)
        case 500:
            raise TributeApiV1ServerError(error.error, error.message)
        case 502:
            raise TributeApiV1BadGateway(error.error, error.message)
        case _:
            raise TributeApiV1Exception(error.error, error.message)
