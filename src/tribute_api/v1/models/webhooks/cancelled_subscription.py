...
class TributeCancelledSubscriptionPayload(BaseModel):
    """Notification about user cancelling a subscription."""

    subscription_name: str = Field(..., examples=["Join the private club 🎉"])
    """Subscription name."""

    subscription_id: int = Field(..., examples=[1646])
    """Subscription ID."""

    period_id: int = Field(..., examples=[1549])
...
class TributeCancelledSubscriptionNotification(TributeBaseNotification):
    """Notification about user cancelling a subscription."""

    name: Literal[TributeNotificationType.CANCELLED_SUBSCRIPTION]
    payload: TributeCancelledSubscriptionPayload
